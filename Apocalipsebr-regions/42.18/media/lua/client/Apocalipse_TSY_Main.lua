...
local function Apocalipse_TSY_RegisterClusterScream(x, y, timeNow)
    ...
    Apocalipse_TSY_ClusterEvents[#Apocalipse_TSY_ClusterEvents + 1] = {
        x = x,
        y = y,
        t = timeNow
    }
...
local function Apocalipse_TSY_ScheduleNearScream(zombie, voiceIndex, vars)
    ...
    Apocalipse_TSY_Delay(function()
        if zombie and not zombie:isDead() and (not zombie.isFakeDead or not zombie:isFakeDead()) and
            (not zombie.isOnFloor or not zombie:isOnFloor()) then
            if useWorld then
                local sq = zombie:getSquare()
                if sq then
                    getSoundManager():PlayWorldSound(soundName, sq, 0, vol, 1.0, false)
...
-- Use public IsoZombie API: SPEED_SPRINTER == 1.
local function Apocalipse_TSY_IsSprinter(zombie)
    return zombie and zombie:getSpeedType() == 1
end
...
local function Apocalipse_TSY_OnTick()
    ...
    if vars and vars.MaxProcessDistance and vars.MaxProcessDistance > 0 then
        maxDist = vars.MaxProcessDistance
    end
    ...
    if vars and vars.EnableNearScream ~= nil then
        enableNear = vars.EnableNearScream
    end
