...
function ISRegionManagerAdminPanel:createChildren()
    ...
    -- Title
    self.titleLabel = ISLabel:new(10, 10, FONT_HGT_MEDIUM, getText("UI_Region_Manager_Panel"), 1, 1, 1, 1,
        UIFont.Medium, true)
    ...
    -- Refresh button
    self.refreshBtn = ISButton:new(10, btnY, btnWid, btnHgt, getText("UI_Region_Manager_Refresh"), self,
        ISRegionManagerAdminPanel.onRefresh)
    ...
    -- New Region button
    self.newRegionBtn = ISButton:new(10 + (btnWid + 10), btnY, btnWid, btnHgt, getText("UI_Region_Manager_New_Region"),
        self, ISRegionManagerAdminPanel.onNewRegion)
    self.newRegionBtn:initialise()
    self.newRegionBtn:instantiate()
    self.newRegionBtn.borderColor = {
        r = 0.3,
        g = 1,
        b = 0.3,
        a = 0.6
    ...
    -- Edit Region button (side-by-side with New Region)
    self.editRegionBtn = ISButton:new(10 + (btnWid + 10) * 2, btnY, btnWid, btnHgt,
        getText("UI_Region_Manager_Edit_Region"), self, ISRegionManagerAdminPanel.onEditRegion)
    self.editRegionBtn:initialise()
    self.editRegionBtn:instantiate()
    self.editRegionBtn.borderColor = {
        r = 0.3,
        g = 0.7,
        b = 1,
        a = 0.7
    }
    self:addChild(self.editRegionBtn)

    local isAdmin = isPlayerAdmin(getPlayer())
    self.newRegionBtn:setVisible(isAdmin)
    self.editRegionBtn:setVisible(isAdmin)

    -- Close button
    self.closeBtn = ISButton:new(self.width - btnWid - 10, btnY, btnWid, btnHgt, getText("UI_Region_Manager_Close"),
        self, ISRegionManagerAdminPanel.onClose)
...
function ISRegionManagerAdminPanel:drawRegionListItem(y, item, alt)
    ...
    local statusColor = region.enabled and {
        r = 0.3,
        g = 1,
        b = 0.3
    } or {
...
function ISRegionManagerAdminPanel:populateList()
    ...
    if RegionManager and RegionManager.Client and RegionManager.Client.zoneData then
        ...
        if #zoneData > 0 then
            ...
            local hiddenAutoCount = 0
            for i, zone in ipairs(zoneData) do
                if zone.isAutoGenerated then
                    hiddenAutoCount = hiddenAutoCount + 1
                else
                    ...
                    local region = {
                        ...
                        customProperties = zone.customProperties or {},
                        isAutoGenerated = zone.isAutoGenerated == true
                    }

                    self.regionList:addItem(displayName, {
                        region = region,
                        bounds = zone.bounds
                    })
            ...
            print("[RegionManager Admin] Populated list with " .. visibleCount ..
                      " editable zones from server (hidden auto-generated: " .. hiddenAutoCount .. ")")
            return
        end
    end
    ...
    if RegionManager and RegionManager.Config and RegionManager.Config.Regions then
        for i, region in ipairs(RegionManager.Config.Regions) do
            local isAutoGenerated = false
            if region.id and string.find(region.id, "^AutoSafeZone_") then
                isAutoGenerated = true
            end
            for _, cat in ipairs(region.categories or {}) do
                if cat == "AUTOSAFE" then
                    isAutoGenerated = true
                    break
                end
            end
...
function ISRegionManagerAdminPanel:onEditRegion()
    if not isPlayerAdmin(getPlayer()) then
        return
    end
    ...
    if not region or region.isAutoGenerated then
        if player then
            player:Say("Auto-generated regions are not editable.", 1, 0.4, 0.4, UIFont.Small, 2, "radio")
        end
        return
    end

    RegionManager.MapPicker.OpenForEdit(region)
    self:close()
end
...
function RegionManager.OnFillWorldObjectContextMenu(playerNum, context, worldobjects, test)
    context:addOption(getText("UI_Regions_Panel"), nil, RegionManager.AdminPanel.Open)
end

Events.OnFillWorldObjectContextMenu.Add(RegionManager.OnFillWorldObjectContextMenu)
