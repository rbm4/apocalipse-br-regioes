...
---@return boolean
local function isAutoGeneratedRegion(region)
    if not region then
        return false
    end
...
---@return table
local function buildZoneBoundariesPayload()
    local zoneList = {}
    for _, data in pairs(RegionManager.Server.registeredZones or {}) do
        table.insert(zoneList, {
            ...
            categories = data.region.categories or {},
            customProperties = data.region.customProperties or {},
            enabled = data.region.enabled ~= false,
            isAutoGenerated = isAutoGeneratedRegion(data.region)
        })
    end
...
local function OnClientCommand(module, command, player, args)
    ...
    if command == "RequestAllBoundaries" then
        ...
        if not payload then
            payload = buildZoneBoundariesPayload()
            RegionManager.Server._cachedBoundariesPayload = payload
            print("Apocalipse_TSY: Built zone boundaries payload (" .. #payload.zones .. " zones, cached)")
        end
