...
local ToughnessTrackedZones = {}
local ToughnessTrackedZoneCount = 0
local ToughnessZoneTrackerRegistered = false

local function zoneHasToughnessChance(zoneData)
    if not zoneData then
        return false
    end

    local props = zoneData.customProperties or zoneData.properties or zoneData
    local chance = tonumber(props and props.toughnessChance) or 0
    return chance > 0
end

local function addTrackedToughZone(zoneId, zoneData)
    if not zoneId or not zoneHasToughnessChance(zoneData) then
        return
    end

    if not ToughnessTrackedZones[zoneId] then
        ToughnessTrackedZones[zoneId] = true
        ToughnessTrackedZoneCount = ToughnessTrackedZoneCount + 1
    end
end

local function removeTrackedToughZone(zoneId)
    if zoneId and ToughnessTrackedZones[zoneId] then
        ToughnessTrackedZones[zoneId] = nil
        ToughnessTrackedZoneCount = math.max(0, ToughnessTrackedZoneCount - 1)
    end
end

local function rebuildTrackedToughZonesFromCurrent()
    ToughnessTrackedZones = {}
    ToughnessTrackedZoneCount = 0

    local clientTick = RegionManager and RegionManager.ClientTick
    local getCurrentZones = clientTick and clientTick.getCurrentZones
    if not getCurrentZones then
        return
    end

    local currentZones = getCurrentZones()
    if not currentZones then
        return
    end

    for zoneId, zoneData in pairs(currentZones) do
        addTrackedToughZone(zoneId, zoneData)
    end
end

local function registerToughnessZoneTracker()
    if ToughnessZoneTrackerRegistered or isServer() then
        return
    end

    local clientTick = RegionManager and RegionManager.ClientTick
    if not clientTick or not clientTick.registerModule then
        return
    end

    clientTick.registerModule({
        name = "Apocalipse_TSY_ToughnessTracker",
        onZoneEntered = function(player, zoneId, zoneData)
            addTrackedToughZone(zoneId, zoneData)
        end,
        onZoneExited = function(player, zoneId, zoneData)
            removeTrackedToughZone(zoneId)
        end
    })

    ToughnessZoneTrackerRegistered = true
    rebuildTrackedToughZonesFromCurrent()
end

local function isPlayerInToughnessRegion(player)
    if not player then
        return false
    end

    if not ToughnessZoneTrackerRegistered then
        registerToughnessZoneTracker()
    end

    return ToughnessTrackedZoneCount > 0
end
...
local function OnWeaponHitCharacter(attacker, target, weapon, damage)
    ...
    local isKnownTough = toughnessType == "tough"
    local isExhaustedTough = toughnessType == "exhausted"

    -- Exhausted zombies must never re-enter the provisional shield path.
    if isExhaustedTough then
    ...
    local isToughRegion = isPlayerInToughnessRegion(player)
    if not isKnownTough and not isToughRegion then
        if target:avoidDamage() then
            target:setAvoidDamage(false)
        end
        return
    ...
    if isKnownTough and hitCounter >= maxHits then
        return
    end

    -- Provisional shielding should happen only once per zombie when we do not
    -- yet know if it's tough. Known tough zombies still shield on each valid hit.
    local provisionalAlreadyApplied = modData.Apocalipse_TSY_ProvisionalShieldApplied == true
    local shouldApplyShieldNow = isKnownTough or (not provisionalAlreadyApplied)
    if shouldApplyShieldNow then
        target:setAvoidDamage(true)
        forceResistedHitStance(target)
        if not isKnownTough then
            modData.Apocalipse_TSY_ProvisionalShieldApplied = true
        end
    end
    ...
    if not isKnownTough then
        print("Apocalipse_TSY: Provisional tough-hit guard applied in toughness-capable region for pid=" ..
                  tostring(targetPID) .. " onlineID=" .. tostring(zombieID))
    end
end

-- Register the event handler (client-side only)
if not isServer() then
    if not RegionManager or not RegionManager.ClientTick then
        pcall(function()
