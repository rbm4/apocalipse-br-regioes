...
local function Apocalipse_TSY_OnClientCommand(module, command, player, args)
    ...
    if command == "ClearZombieStates" then
        if player and player:getAccessLevel() ~= "None" then
            ...
            sendServerCommand(player, "Apocalipse_TSY", "ClearConfirm", {count = count})
            print("Apocalipse_TSY Server: Admin " .. player:getUsername() ..
                  " cleared zombie ModData (" .. count .. " entries)")
        else
            print("Apocalipse_TSY Server: Non-admin " .. player:getUsername() ..
    ...
    if command == "RequestZombieInfo" and player and args.zombies then
        ...
        for _, request in ipairs(args.zombies) do
            ...
            if stored and request.x and request.y then
                ...
                if dx > 300 or dy > 300 then
                    -- print("Apocalipse_TSY Server: Zombie " .. persistentID ..
            ...
            if stored then
                ...
                -- packets, ensuring all clients receive the correct speed regardless of
                -- which client currently owns the zombie.
                -- if stored.isSprinter or stored.isShambler then
                --     local cell = getCell()
                --     if cell then
                --         local serverZombies = cell:getZombieList()
                --         if serverZombies then
                --             for j = 0, serverZombies:size() - 1 do
                --                 local sz = serverZombies:get(j)
                --                 if sz and sz:getOnlineID() == zombieID then
                --                     local szModData = sz:getModData()
                --                     if stored.isSprinter then
                --                         szModData.Apocalipse_TSY_LockedSpeed = 1
                --                     elseif stored.isShambler then
                --                         szModData.Apocalipse_TSY_LockedSpeed = 3
                --                     end
                --                     break
                --                 end
                --             end
        ...
        if found > 0 or notFound > 0 then
            -- print("Apocalipse_TSY Server: Sent info for " .. found .. " zombies, " ..
